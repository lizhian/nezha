...
function normalizeAttachmentPasteSettings(settings: Partial<AppSettings>): TerminalAttachmentPasteSettings {
  return {
    enabled:
...
function formatTerminalAttachmentPaste(attachments: SavedTerminalAttachment[]): string {
  return (
    attachments
      .map((attachment) => `[Attachment ${attachment.label}](${attachment.path})`)
...
export function attachTerminalFilePaste({
  ...
  const handlePaste = (event: ClipboardEvent) => {
    ...
    Promise.all(files.map(fileToAttachment))
      .then((files) =>
